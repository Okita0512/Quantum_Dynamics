...
tau_c = 500 * fs_to_au                       # cavity lifetime
gamma_c = 1e10 * cm_to_au                   # loss bath cutoff frequency
rescaling = 0.5

n_0 = 1 / (np.exp(beta * w0) - 1) * 0.5
print('n_0(w0) = ', n_0)
...
for z in wc_scan:
    ...
    rate_w = (np.pi * lamc_mu_0**2 * z * gauss(z, w0, sigma_2) * Bose(wc) / (1 + np.pi * lamc_mu_0**2 * z * gauss(z, w0, sigma_2) * tau_c)) * rescaling
...
plt.plot(x_axis, rate_1, linewidth = lw, color = 'cyan', label = r"$\Omega_\mathrm{R} = 6.27~\mathrm{cm}^{-1}$")
plt.plot(x_axis, rate_2, linewidth = lw, color = 'green', label = r"$12.54~\mathrm{cm}^{-1}$")
plt.plot(x_axis, rate_3, linewidth = lw, color = 'red', label = r"$25.09~\mathrm{cm}^{-1}$")

plt.text(1360, 1.3, r'$\omega_\mathrm{0}$', color = 'black', size = 20)
plt.text(1270, 1.25, r'$1172\ \mathrm{cm}^{-1}$', color = 'black', size = 20)

# x and y range of plotting 
x1, x2 = 600, 1740
y1, y2 = 0.95, 1.4     # y-axis range: (y1, y2)
...
plt.text(750, 1.2, r'$\times$0.5', color = 'goldenrod', size = 24)
plt.text(1360, 1.3, r'$\omega_\mathrm{0}$', color = 'black', size = 20)
plt.text(1270, 1.25, r'$1172\ \mathrm{cm}^{-1}$', color = 'black', size = 20)
...
ax.vlines([1172], 0.95, 1.4, linestyles = 'dashed', colors = ['black'], lw = 3.0) 
