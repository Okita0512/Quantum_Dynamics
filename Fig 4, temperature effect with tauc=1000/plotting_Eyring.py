...
for t in Temp:
    ...
    data1 = np.loadtxt("./PSD_es=0.1_ec=0.00625_tauc=1000_temp=%d.txt" % (Temp[count]), dtype = float)
    data2 = np.loadtxt("./PSD_es=0.1_ec=0.0125_tauc=1000_temp=%d.txt" % (Temp[count]), dtype = float)
    data3 = np.loadtxt("./PSD_es=0.1_ec=0.025_tauc=1000_temp=%d.txt" % (Temp[count]), dtype = float)
    data4 = np.loadtxt("./PSD_es=0.1_ec=0.05_tauc=1000_temp=%d.txt" % (Temp[count]), dtype = float)
    data5 = np.loadtxt("./PSD_es=0.1_ec=0.1_tauc=1000_temp=%d.txt" % (Temp[count]), dtype = float)
...
count = 0
for etac in ec_plot:
    lamc_mu_0 = mu * np.sqrt(2 * wc) * etac
    result = np.pi * lamc_mu_0**2 * wc * gauss(wc, w0, sigma_2) / (1 + np.pi * lamc_mu_0**2 * wc * gauss(wc, w0, sigma_2) * tauc) * Bose(wc)
...
y1, y2 = -0.05, 0.45     #-15, -10 # y-axis range: (y1, y2)
...
plt.savefig("Fig_temp_Eyring.pdf", bbox_inches='tight')