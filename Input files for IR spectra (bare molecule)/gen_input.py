...
def get_rho0(NStates):
    ...
    for i in range(NStates):
        for j in range(NStates):
            ...
            rho0[i, j] = 1.0 * delta(i, 0) * delta(j, 0)
    
    return np.dot(Rs, rho0)
...
if __name__ == '__main__':
    ...
    hams = get_Hs(NStates, lambda_1, RE_c = 0.0)    # turn off the effective bath
...
# ==============================================================================================================================
    ...
    jomg = ini['bath']['jomg']
    nind = 0
    for m in range(nmod - 1):   # one mode is Brownian
        try:
    ...
    jomg = ini['bath']['jomg']
    mode = np.zeros((nind + 2 + npsd + r), dtype = int)
    for i in range(nind, nind + npsd + 2 + r):
        mode[i] = 1
    ...
    qmds[1,:,:] = 0.0 # Qs2                           # the effective spectral density for the cavity, turned off
...
# ==============================================================================================================================
    ...
    pdip = np.zeros((nmod,NStates,NStates),dtype=float)
    pdip[0,:,:] = np.identity(NStates)
