...
ax.legend(ncol = 4, loc = 'lower center', frameon = True, edgecolor = 'black', fancybox = False, borderpad = 0.7, prop = font_legend, bbox_to_anchor=(0.5, 0.979))
...
plt.tick_params(axis = 'x', which = 'both', direction = 'in', labelsize = 30)
...
# ax.set_xlabel(r'$\mathcal{J}_{\nu}(\omega)\ \mathrm{Intensity}\ (\times 10^{-4}\ \mathrm{a.u.})$', size = 23)
...
ax2.tick_params(which = 'major', length = 15, pad = 0.5)
ax2.tick_params(which = 'minor', length = 5)
...
plt.xlim(- 0.25, j_0)
ax.set_xlabel(r'$\mathcal{J}\mathrm{_{eff}}(\omega)\ \mathrm{Intensity}\ (\times 10^{-6}\ \mathrm{a.u.})$', size = 23, labelpad = 15)
...
# ax.plot(Drude(w2, lambda_1, gamma_1) / ratio2, w2 / cm_to_au, linewidth = lw, color = 'navy') # , label = r"$\mathcal{J}\mathrm{_{diss}}(\omega)$"
ax.plot(gen_jw(w1, omega_c, eta_c, λc(tau_c, omega_c, gamma_2, temp), gamma_2) / ratio1, w1 / cm_to_au, linewidth = lw, color = 'red')
# , label = r"$\mathcal{J}\mathrm{_{eff}}(\omega)$"
...
plt.text(3, 1350, r'$\mathcal{J}\mathrm{_{eff}}(\omega)$', color = 'red', fontsize = 32)
# plt.text(2, -400, r'$\mathcal{J}_{\nu}(\omega)$', color = 'navy', fontsize = 32)
