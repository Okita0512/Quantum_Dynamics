...
# data1 = np.loadtxt("./Discrete_es=0.1_ec=0.05_tauc=inf.txt", dtype=float)
# plt.plot(data1[:, 0], data1[:, 1] / rate_0, linewidth = lw, color = 'cyan', label = r"$\tau_c \rightarrow \infty$", alpha = .8)
# rate_collection.append(data1[10, 1])
...
# data1 = np.loadtxt("./PSD_es=0.1_ec=0.05_tauc=5000.txt", dtype=float)
# data2 = np.loadtxt("./PSD_es=0.1_ec=0.025_tauc=5000.txt", dtype=float)
# data3 = np.loadtxt("./PSD_es=0.1_ec=0.0125_tauc=5000.txt", dtype=float)
# rate_collection_1.append(data1[10, 1])
# rate_collection_2.append(data2[1])
# rate_collection_3.append(data3[1])
...
# create the FGR results and plot
alpha = 0.5                                                     # ad hoc correction parameter
k0_HEOM = 1.267242438468303195e-07                              # bare molecule rate
...
for z in tauc_data1:
    ...
    rate_wR = k1 * k2 / (k1 + k2)
    rate_L.append(1 + alpha * rate_wL / k0_HEOM)
    rate_R.append(1 + alpha * rate_wR / k0_HEOM)
    rate_wC = rate_wL * rate_wR / (rate_wL + rate_wR)
    rate_C.append(1 + alpha * rate_wC / k0_HEOM)

plt.semilogx(tauc_data1, rate_C, '-', linewidth = 3, color = 'red')
...
plt.semilogx(tauc_data1, rate_C, '-', linewidth = 3, color = 'greenyellow')
...
plt.semilogx(tauc_data1, rate_C, '-', linewidth = 3, color = 'navy')

# plot the HEOM results
k_0 = 1.267393751572242088e-07
plt.semilogx(data[:, 0], data[:, 3] / k_0, 'o-', markersize = 15, linewidth = 1, markerfacecolor = 'white', color = 'red', label = '$\Omega_\mathrm{R} =$ 6.27 cm$^{-1}$')
plt.semilogx(data[:, 0], data[:, 2] / k_0, 'o-', markersize = 15, linewidth = 1, markerfacecolor = 'white', color = 'greenyellow', label = '12.54 cm$^{-1}$')
plt.semilogx(data[:, 0], data[:, 1] / k_0, 'o-', markersize = 15, linewidth = 1, markerfacecolor = 'white', color = 'navy', label = '25.09 cm$^{-1}$')
...
# plt.text(760, 1.5, r'$\omega_\mathrm{b}$', color = 'green', size = 20)
# plt.text(650, 1.4, r'$1000\ \mathrm{cm}^{-1}$', color = 'green', size = 20)
...
ax.vlines([211.6], 0.95, 1.4, linestyles = 'dashed', colors = ['navy'], lw = 3.0) 
ax.vlines([2 * 211.6], 0.95, 1.2, linestyles = 'dashed', colors = ['greenyellow'], lw = 3.0) 
ax.vlines([4 * 211.6], 0.95, 1.1, linestyles = 'dashed', colors = ['red'], lw = 3.0) 
